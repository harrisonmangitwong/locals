...
const FOOD_SKIP = new Set([
  "that", "this", "just", "with", "from", "they", "their", "here", "have", "also",
  "really", "very", "some", "more", "much", "many", "most", "such", "what", "when",
  "which", "while", "been", "were", "will", "would", "could", "should", "about",
  "into", "than", "then", "them", "there", "these", "those", "after", "before",
  "place", "food", "meal", "service", "staff", "time", "dish", "spot", "night",
  "everything", "something", "anything", "nothing", "definitely", "absolutely",
]);

function extractDishes(reviews: ReviewEntry[]): string[] {
  const pattern = /(?:try|order(?:ed)?|get|got|had|ate|love[d]?|recommend(?:ed)?)\s+(?:the\s+|their\s+)?([a-z][a-z\s-]{3,28}?)(?=\s+(?:was|were|is|are|and|but|,|\.|!|\?))/gi;
  const counts: Record<string, number> = {};
  for (const rev of reviews) {
    pattern.lastIndex = 0;
    let match: RegExpExecArray | null;
    while ((match = pattern.exec(rev.text)) !== null) {
      const dish = match[1].trim().toLowerCase();
      const words = dish.split(/\s+/);
      if (words.some(w => FOOD_SKIP.has(w))) continue;
      if (words.length > 4 || dish.length < 4) continue;
...
export default function RestaurantPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col" style={{ backgroundColor: "var(--bg)" }}>
        ...
        <div className="w-full max-w-2xl mx-auto px-4 sm:px-6 pt-6 space-y-3">
  ...
  const dishes = extractDishes(reviews);
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: "var(--bg)", color: "var(--text)" }}>
      ...
      <div className="relative w-full overflow-hidden" style={{ height: "clamp(260px, 40vw, 420px)" }}>
        ...
        <button
          ...
          className={`heart-btn absolute top-4 right-4 flex items-center justify-center w-9 h-9 rounded-full${savePop ? " heart-pop" : ""}`}
      ...
      {/* Action bar */}
      <div className="flex" style={{ borderBottom: "1px solid var(--border)", backgroundColor: "var(--bg)" }}>
        <a
          href={r.url}
          target="_blank"
          rel="noopener noreferrer"
          className="flex-1 flex items-center justify-center gap-1.5 py-3.5 text-sm font-medium transition-opacity hover:opacity-75"
        ...
        {r.phone && (
          <a
            href={`tel:${r.phone}`}
            className="flex-1 flex items-center justify-center gap-1.5 py-3.5 text-sm font-medium transition-opacity hover:opacity-75"
            style={{ color: "var(--text-secondary)", borderRight: r.website ? "1px solid var(--border)" : undefined }}
          >
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* Body */}
      <main className="flex-1 w-full max-w-2xl mx-auto px-4 sm:px-6 py-8 space-y-10">

        {/* Known for */}
        {dishes.length > 0 && (
          <section>
            <h2 className="text-xs font-semibold tracking-widest uppercase mb-3" style={{ color: "var(--text-muted)", letterSpacing: "0.1em" }}>Known for</h2>
        ...
        {/* Local take */}
        <section>
          <h2 className="text-xs font-semibold tracking-widest uppercase mb-3" style={{ color: "var(--text-muted)", letterSpacing: "0.1em" }}>The local take</h2>
        ...
        {topReviews.length > 0 && (
          <section>
            <h2 className="text-xs font-semibold tracking-widest uppercase mb-4" style={{ color: "var(--text-muted)", letterSpacing: "0.1em" }}>What locals say</h2>
        ...
        {hasDetails && (
          <section style={{ borderTop: "1px solid var(--border)", paddingTop: "2rem" }}>
            ...
            >
              <h2 className="text-xs font-semibold tracking-widest uppercase" style={{ color: "var(--text-muted)", letterSpacing: "0.1em" }}>Address & hours</h2>
              <svg
